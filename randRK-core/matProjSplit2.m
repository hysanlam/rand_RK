function Y = matProjSplit2(Y,F,t0,t1)
    ...
    % K-step
    K = U*S;
    K = odeSolver(K, @(X) funK(X,V),t0,t0+dt./2);
   %K = rk4(K, @(X) funK(X,V),dt);
    [U,S] = qr(K,0);
    
    %S-step
    S=odeSolver(S, @(X) -funS(X,U,V),t0,t0+dt./2);
    ...
    [V,S] = qr(L,0);
    S = S';
    S=odeSolver(S, @(X) -funS(X,U,V),t0+dt./2,t1);

    K = U*S;
    K = odeSolver(K, @(X) funK(X,V),t0+dt./2,t1);
    [U,S] = qr(K,0);
    Y = {U,V,S};
end
